# 🚨 Projeto: Alarmes Inteligentes e a aplicação de Redes Neurais em Computação Visual.

## 🎓 Disciplinas

Inteligência Artificial e Visão Computacional.

## 👥 Grupo

⚠️ **ATENÇÃO:** No grupo, está sinalizada a(s) disciplina(s) que cada membro está executando para o trabalho.

**Legenda:**

- IA: Inteligência Artificial
- CV: Computação Visual

**Integrantes**

- Lucas Tuon de Matos - 10417987 (CV)
- Matheus de Andrade Lourenço - 10419691 (IA/CV)
- Murillo Cardoso Ferreira - 10418082 (IA/CV)
- Pietro Zanaga Neto - 10418574 (IA/CV)

## 💡 Proposta do Projeto

Esse projeto consiste em um tutorial teórico e prático sobre o funcionamento das Redes Neurais na área de Computação Visual, em conjunto com um exemplo de implementação de um projeto sobre alarmes inteligentes.

## 🗂️ Organização do Repositório

> 📁 1_TEORIA
    Contém arquivo .md com síntese teórica sob o tema. Comece em [contexto_teorico.md](contexto_teorico.md)

> 📁 2_PRÁTICA
    Contém projeto de exemplo de implementação de alertas inteligentes. Comece em [0_informacoes_projeto.md](0_informacoes_projeto.md)

## 🧩 Responsabilidades do Grupo

- Lucas Tuon de Matos
    - Implementação da Demonstração do Modelo
- Matheus de Andrade Lourenço
    - Gerenciamento e formatação do Repositório
    - Implementação de Avaliação do Modelo
    - Implementação dos Alertas (2 níveis)
- Murillo Cardoso Ferreira
    - Implementação de Notebook de Análise Exploratória
- Pietro Zanaga Neto
    - {INSERIR AQUI}
    - ...

## 🎥 Link do Vídeo de Apresentação

{INSERIR AQUI}